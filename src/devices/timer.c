...
timer_sleep (int64_t ticks) 
{
  ...
  /* Put the current thread to sleep instead of busy-waiting.
     The sleep list is shared with the timer interrupt handler,
     which cannot acquire locks, so guard it by disabling
     interrupts.  While interrupts stay off, thread_block() cannot
     be interrupted between inserting the thread and blocking it,
     so no wake-up is ever missed. */
  old_level = intr_disable ();
  cur = thread_current ();
  cur->wake_time = timer_ticks () + ticks;
  list_insert_ordered (&sleeping_list, &cur->elem, timer_sleep_less, NULL);
  thread_block ();
  intr_set_level (old_level);
}
